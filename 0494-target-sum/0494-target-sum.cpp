class Solution {
public:
    int findTargetSumWaysUtil(vector<int>& nums, int ind, int target, vector<vector<int>> &dp){
    // Base cases
    if (ind == 0) {
    ...
    int notTaken = findTargetSumWaysUtil(nums, ind - 1, target, dp);
    
    // Calculate the number of ways by taking the current element
    int taken = 0;
    if (nums[ind] <= target)
        taken = findTargetSumWaysUtil(nums, ind-1, target-nums[ind], dp);
    ...
    return findTargetSumWaysUtil(nums, n-1, s2, dp); // Call the helper function
